      ...
      <main className='flex justify-center h-screen w-full'>
        <div className='lg:w-[948px] border-x w-full flex flex-col justify-start items-center z-10'>
          <div className='h-16 w-full flex items-center justify-between text-sm text-center sm:text-left py-4 px-2 border-b font-semibold bg-white dark:bg-[#232323]'>
            <div>⚡ZEUS</div>
            {!isDarkTheme ? <MoonStar onClick={() => setTheme('dark')} size={18} className='cursor-pointer mr-2' /> : <Sun onClick={() => setTheme('light')} size={18} className='cursor-pointer mr-2' />}
          </div>
          <div className='w-full flex flex-col gap-3 justify-center items-center' style={{ backgroundSize: '14px 14px', backgroundImage: `linear-gradient(to right,${isDarkTheme ? '#272727' : '#f6f6f6'} 1px,transparent 1px),linear-gradient(to bottom,${isDarkTheme ? '#272727' : '#f6f6f6'} 1px,transparent 1px)` }}>
