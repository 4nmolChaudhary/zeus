      ...
      <ResponsiveDialog title='Settings' description='Update your preferences' open={open} onOpenChange={setOpen}>
        <div className='grid grid-cols-2 md:grid-cols-4 mt-3 gap-2'>
          {items?.map((item, index) => (
            <div key={item} className={`flex items-center flex-col justify-center rounded-sm py-2 cursor-pointer ${index === 5 ? 'md:col-span-3 col-span-2' : 'md:aspect-square'} ${index === 4 ? `md:col-span-1 col-span-2 bg-rose-900 border border-rose-600 hover:bg-rose-600/50` : `bg-secondary border hover:bg-background/50`}`}>
              {index !== 5 ? (
                <>
                  {icons[item]}
